...
export function registerRoutes(app: Express) {
  ...
  app.post("/api/login", async (req, res) => {
    try {
      ...
      if (user.cooldownUntil && user.cooldownUntil > new Date()) {
        ...
        return res.status(429).json({ message: "Account temporarily locked.", remainingSeconds: remaining });
      }
  ...
  app.post("/api/register", async (req, res) => {
    try {
      ...
      if (error instanceof ZodError) return res.status(400).json({ message: "Invalid request data", errors: error.errors });
      if (error.message === "Username already exists") return res.status(409).json({ message: error.message });
  ...
  app.post("/api/admin/reset-password", async (req, res) => {
    try {
      ...
      if (!adminUser || adminUser.role.toLowerCase() !== "admin") return res.status(403).json({ message: "Access denied" });
  ...
  // Keep all existing routes for products, sales, stock deduction, and reports
