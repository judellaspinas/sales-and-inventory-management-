...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // RESET STAFF OR SUPPLIER PASSWORD
  ...
  app.get("/api/admin/accounts", async (req, res) => {
    try {
      ...
      const filtered = allUsers.filter(
        (u) => u.role.toLowerCase() === "staff" || u.role.toLowerCase() === "supplier"
      );
...
// ✅ Create new sale (deduct stock + record sale)
app.post("/api/sales", async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      // Deduct from stock and record sale
...
export const routes = [
  ...
  { path: "/reset-staff-password", component: ResetStaffPassword },
