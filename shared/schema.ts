// src/shared/schema.ts (the file you posted)
...
export const users = pgTable("users", {
  ...
  email: text("email"),
  phone: text("phone"),
  role: text("role").notNull().default("Admmin"),
  supply: text("supply"),
...
export const orders = pgTable("orders", {
  ...
  status: text("status")
    .notNull()
    .default("pending"), // ✅ pending, confirmed, cancelled only
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
...
export const registerSchema = z
  .object({
    ...
    // allow form strings to be coerced into number automatically
...
export const updateOrderSchema = z.object({
  status: z.enum(["pending", "confirmed", "cancelled"]).optional(), // ✅ Updated enum
